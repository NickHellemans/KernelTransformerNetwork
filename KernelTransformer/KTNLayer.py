...
from KernelTransformer.KTN import KTN_ARCHS
from KernelTransformer.SphereProjection import SphereProjection
from KernelTransformer.util import create_variable
...
class KTNConv(nn.Module):
    ...
    def forward(self, X, rows=None):
        ...
        outputs = create_variable(size, X.device)
...
class KTNConvScratch(KTNConv):
    def __init__(self, input_channel, output_channel, kernel_size,
        ...
        super().__init__(conv.weight.clone(), conv.bias.clone(), sphereH, imgW, fov, dilation, tied_weights, arch,
                         kernel_shape_type)



def compute_kernelshape(kernel_size=3, sphereH=320, fov=65.5, imgW=640, dilation=1, tied_weights=5,
                        kernel_shape_type="dilated"):
    sphereW = sphereH * 2
